...
namespace AdminEmpPortal.Services
{
    class Accountservice : IAccountservice
    {
        ...
        private BearerToken GetToken(ApplicationUser AppUser)
        {
            ...
            var Claims = _context.IdentityUserClaims.Where(x => x.UserId == AppUser.Id && (x.ClaimValue == "Employee" || x.ClaimValue == "Admin")).ToList();
            foreach (var Claim in Claims)
            {
                if (claims.Any(c => Claim.ClaimType.Equals(c.Type, StringComparison.InvariantCultureIgnoreCase)) == false)
                {
